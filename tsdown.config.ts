/**
 * dsh-wx-skin tsdown config — emits only the browser client bundle.
 * The host half is emitted by tsc (tsconfig.json → lib/*.js + lib/types).
 *
 * The client bundle is a closure-factory artifact matching the running DSH
 * shell's module loader contract (mirrors packages/client/tsdown.client.ts):
 * it calls window.__ModuleLoader__.load({id, factory}) and resolves externals
 * through the injected require (the loader module table — the platform module
 * list below mirrors packages/client/web/src/platform.ts). CSS Modules are
 * compiled by lightningcss inside the bundle; the css text auto-injects a
